...
in
  stdenvNoCC.mkDerivation {
    ...
    passthru.updateScript = writeScript "update-paper" ''      #! /usr/bin/env nix-shell
      ...
      curl -L https://papermc.io/api/v2/projects/paper/versions/1.20/builds | jq '{"version":.version, "build":.builds[-1].build, "name":.builds[-1].downloads.application.name, "sha256":.builds[-1].downloads.application.sha256}' > minecraft/papermc/paper.json
    '';
  }
